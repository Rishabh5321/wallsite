...
export function buildFileTree(galleryData) {
    ...
    const allFolderLi = createTreeElement({ name: 'All', type: 'folder', children: [galleryData] });
...
function createTreeElement(node) {
    ...
    const chevronIcon = hasSubfolders ? '<svg class="chevron" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg>' : '';
    ...
    itemDiv.innerHTML = `
        <span class="icon">${chevronIcon}${folderIcon}${folderOpenIcon}</span>
        <span class="name">${node.name}</span>
    `;
    // Prepend chevron for correct layout
    if(hasSubfolders) {
        itemDiv.querySelector('.icon').insertAdjacentHTML('beforebegin', chevronIcon);
    }
