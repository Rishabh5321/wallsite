(() => {
							...
							function (e, n, t) {
								...
								const i = function (e2) {
									return (
										...
										e2.ownerDocument.body.contains(e2) ===
											true
									);
								};
								t.visible = i;
								t.create = function (e2, n2) {
									const t2 = (function (e3, n3) {
										...
										(e2 = (function (e3) {
											const n3 = typeof e3 === 'string';
											const t3 =
												e3 instanceof HTMLElement == 1;
											if (n3 === false && t3 === false)
												throw new Error(
													'Content must be a DOM element/node or string'
												);
											return n3 === true
												? Array.from(o(e3, true))
												: e3.tagName === 'TEMPLATE'
													? [
															e3.content.cloneNode(
																true
															),
														]
													: Array.from(e3.children);
										})(e2)),
										(n2 = (function () {
											let e3 =
												arguments.length > 0 &&
											...
											if (
												typeof e3.className !== 'string'
											)
												throw new Error(
													'Property `className` must be a string'
												);
											if (typeof e3.onShow !== 'function')
												throw new Error(
													'Property `onShow` must be a function'
												);
											if (
												typeof e3.onClose !== 'function'
											)
												throw new Error(
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		const randomWallpaperBtn = document.getElementById(
		...
		function setupEventListeners() {
			...
			if (randomWallpaperBtn) {
				randomWallpaperBtn.addEventListener(
					'click',
					showRandomWallpaper
				);
			}
		...
		function updateLightbox(wallpaper) {
			...
			newImg.onload = () => {
				...
				const downloadBtn =
					lightboxElement.querySelector('.download-btn');
		...
		function showRandomWallpaper() {
			...
			const randomIndex = Math.floor(
				Math.random() * activeWallpapers.length
			);
